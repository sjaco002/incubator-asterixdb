...
public class PushFieldAccessRule implements IAlgebraicRewriteRule {
    ...
    private Pair<ILogicalExpression, List<String>> getFieldExpression(AssignOperator access, ARecordType rt)
        ...
        ILogicalExpression e0 = null;
        AbstractFunctionCallExpression accessFun =
                (AbstractFunctionCallExpression) access.getExpressions().get(0).getValue();
        while (AnalysisUtil.isAccessToFieldRecord(accessFun)) {
            ...
            fieldPath.add(0, fldName);
            if (!AnalysisUtil.isAccessToFieldRecord(accessFun.getArguments().get(0).getValue())) {
                e0 = accessFun.getArguments().get(0).getValue();
                break;
            }
            accessFun = (AbstractFunctionCallExpression) accessFun.getArguments().get(0).getValue();
